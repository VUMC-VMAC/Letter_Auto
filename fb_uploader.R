...
#pdb_data <- exportReports(pdb, 241869)
try(pdb_data <- exportReports(pdb, 241869), silent = TRUE)
if (exists("pdb_data")==FALSE) {print("No Updates")} else {
  tm7yr_data <- exportReports(tm7yr, 248514)
  tm60_data <- exportReports(tm60, 248512)
  tm36_data <- exportReports(tm36, 248500)
  tme_data <- exportReports(tme, 248431)
  fii36 <- exportReports(FII, 252306)
  fii60 <- exportReports(FII, 252308)
  fii7yr <- exportReports(FII, 252309)
  
  fii60[which(is.na(fii60["extracardiac_incidental"])),"extracardiac_incidental"]<- "No"
  fii60[which(is.na(fii60["brain_incidental"])),"brain_incidental"]<- "No"
  fii7yr[which(is.na(fii7yr["brain_incidental"])),"brain_incidental"]<- "No"
  fii7yr[which(is.na(fii7yr["extracardiac_incidental"])),"extracardiac_incidental"]<- "No"
  fii36[which(is.na(fii36["extracardiac_incidental"])),"extracardiac_incidental"]<- "No"
  fii36[which(is.na(fii36["brain_incidental"])),"brain_incidental"]<- "No"
  
  #for (i in 1:nrow(pdb_data)) {
  ii<-2
  pdb_data <- pdb_data[ii,]
  ...
  e <- 1+ max(which(ep_sel==FALSE)) # current epoch; might be able to determine from most recent fu_date
  ...
  ind <- map_id
  inddd <- c()
  indd <- find.matches(tm7yr_data[, "map_id"],ind)
  for (i in (1:length(indd$matches))){if (indd$matches[i]>0){inddd<-c(inddd,i)}}
  tm7yr_data[inddd,which(is.na(tm7yr_data[inddd,]))]<- "Missing"
  tm7yr_data <- tm7yr_data[inddd,]
  
  inddd <- c()
  indd <- find.matches(tm60_data[, "map_id"],ind)
  for (i in (1:length(indd$matches))){if (indd$matches[i]>0){inddd<-c(inddd,i)}}
  tm60_data[inddd,which(is.na(tm60_data[inddd,]))]<- "Missing"
  tm60_data <- tm60_data[inddd,]
  
  inddd <- c()
  indd <- find.matches(tm36_data[, "map_id"],ind)
  for (i in (1:length(indd$matches))){if (indd$matches[i]>0){inddd<-c(inddd,i)}}
  tm36_data[inddd,which(is.na(tm36_data[inddd,]))]<- "Missing"
  tm36_data <- tm36_data[inddd,]
  
  inddd <- c()
  indd <- find.matches(tme_data[, "map_id"],ind)
  for (i in (1:length(indd$matches))){if (indd$matches[i]>0){inddd<-c(inddd,i)}}
  tme_data[inddd,which(is.na(tme_data[inddd,]))]<- "Missing"
  tme_data <- tme_data[inddd,]
  
  inddd <- c()
  indd <- find.matches(fii36[, "map_id"],ind)
  for (i in (1:length(indd$matches))){if (indd$matches[i]>0){inddd<-c(inddd,i)}}
  fii36[inddd,which(is.na(fii36[inddd,]))]<- "Missing"
  fii36 <- fii36[inddd,]
  
  inddd <- c()
  indd <- find.matches(fii60[, "map_id"],ind)
  for (i in (1:length(indd$matches))){if (indd$matches[i]>0){inddd<-c(inddd,i)}}
  fii60[inddd,which(is.na(fii60[inddd,]))]<- "Missing"
  fii60 <- fii60[inddd,]
  
  inddd <- c()
  indd <- find.matches(fii7yr[, "map_id"],ind)
  for (i in (1:length(indd$matches))){if (indd$matches[i]>0){inddd<-c(inddd,i)}}
  fii7yr[inddd,which(is.na(fii7yr[inddd,]))] <- "Missing"
  fii7yr <- fii7yr[inddd,]
  i<-1
  ...
  df <- data.frame(
    ...
    ER = c(tme_data[i, 2], paste0(tme_data[i, 3],"/",tme_data[i, 4]), paste0(pdb_data[i, "height_total"]," inches"), paste0(pdb_data[i, "weight"]," lbs"), round(((pdb_data[i, "weight"]/2.205)/((pdb_data[i, "height_total"]/0.393701)/100)^2), digits=1)),
    MR_36 = c(tm36_data[i, 2], paste0(fii36[i, 4],"/",fii36[i, 5]), paste0(round(pdb_data[i, "height_36mos"]*0.393701), " inches"), paste0(round(pdb_data[i, "weight_36mos"]*2.205), " lbs"), round((pdb_data[i, "weight_36mos"]/(pdb_data[i, "height_36mos"]/100)^2), digits=1)),
    MR_60 = c(tm60_data[i, 2], paste0(fii60[i, 4],"/",fii60[i, 5]), paste0(round(as.integer(pdb_data[i, "height_60mos"])*0.393701), " inches"), paste0(round(as.integer(pdb_data[i, "weight_60mos"])*2.205), " lbs"), round((as.integer(pdb_data[i, "weight_60mos"])/(as.integer(pdb_data[i, "height_60mos"])/100)^2), digits=1)),
    CR = c(tm7yr_data[i, 2], paste0(fii7yr[i, 4],"/",fii7yr[i, 5]), paste0(round(as.integer(pdb_data[i, "height_7yr"])*0.393701)," inches"), paste0(round(as.integer(pdb_data[i, "weight_7yr"])*2.205)," lbs"), round((as.integer(pdb_data[i, "weight_7yr"])/(as.integer(pdb_data[i, "height_7yr"])/100)^2), digits=1)),
    NR = c("60-100", "<120 / <80", "n/a", "n/a", "18.5-24.9")
  )
  
  ft <- flextable(df)
  ft <- set_header_labels(ft, ER = paste0("Enrollment Results ",enroll_date), 
  ...
  ft <- fontsize(ft, j=1:6, size = 10, part="header")
  ft <- fontsize(ft, j=1:6, size = 10, part="body")
  ft <- width(ft,j = 2:6, width = 1)
  ft <- width(ft, j=1, width = 1.5)
  ft <- theme_box(ft)
  ft <- align(ft, align = "center", part="header")
  ft <- align(ft, align = "center", part="body")
  ft <- align(ft, i = 1:5, j = 1, align="left",part="body")
  #for (ii in 2:(ncol(df)-1)) {if ((as.integer(df[1,ii]) < 60) | (as.integer(df[1,ii]) >100)) {ft <- bold(ft, i = 1, j = ii, bold = TRUE, part = "body")}
  #if (as.integer(df[2,ii]) > 120) {ft <- bold(ft, i = 2, j = ii, bold = TRUE, part = "body")}
  #if (as.integer(df[3,ii]) > 80) {ft <- bold(ft, i = 3, j = ii, bold = TRUE, part = "body")}
  ...
  ptp_temp <- paste0("resources/output/ptp_temp.docx")
  phys_temp <- paste0("resources/output/phys_temp.docx")
  Plots <- list(membar = function() print(barr))
  ptp_path <- paste0("resources/Templates/Feedback/fb_letter_template_",epoch,".docx")
  phys_path <- paste0("resources/Templates/Feedback/physician_temp_",epoch,".docx")
  ...
  if (val36=="Normal"){val36<-"     2.  No significant"} else {val36<-"     2.  Significant"}
  ...
  if (fii36["extracardiac_incidental"]=="Yes") {lung36<-fii36["extracardiac_incidental_describe"]} else {lung36<-""}
  if (fii60["extracardiac_incidental"]=="Yes") {lung60<-fii60["extracardiac_incidental_describe"]} else {lung60<-""}
  if (fii7yr["extracardiac_incidental"]=="Yes") {lung7yr<-fii7yr["extracardiac_incidental_describe"]} else {lung7yr<-""}
  ...
  if (is.na(first_name_physician1_7yr)==FALSE) {
    first_name_physician <-first_name_physician1_7yr
    last_name_physician <- last_name_physician1_7yr
    street_address_physician <- street_address_physician1_7yr
    city_physician <- city_physician1_7yr
    state_physician <- state_physician1_7yr
    zip_physician <- zip_physician1_7yr
  ...
  if (is.na(first_name_physician2_7yr)==FALSE) {
    first_name_physician <-first_name_physician2_7yr
    last_name_physician <- last_name_physician2_7yr
    street_address_physician <- street_address_physician2_7yr
    city_physician <- city_physician2_7yr
    state_physician <- state_physician2_7yr
    zip_physician <- zip_physician2_7yr
  ...
  if (is.na(first_name_physician3_7yr)==FALSE) {
    first_name_physician <-first_name_physician3_7yr
    last_name_physician <- last_name_physician3_7yr
    street_address_physician <- street_address_physician3_7yr
    city_physician <- city_physician3_7yr
    state_physician <- state_physician3_7yr
    zip_physician <- zip_physician3_7yr
  ...
  if (is.na(first_name_physician4_7yr)==FALSE) {
    first_name_physician <-first_name_physician4_7yr
    last_name_physician <- last_name_physician4_7yr
    street_address_physician <- street_address_physician4_7yr
    city_physician <- city_physician4_7yr
    state_physician <- state_physician4_7yr
    zip_physician <- zip_physician4_7yr
  ...
  if (is.na(first_name_physician5_7yr)==FALSE) {
    first_name_physician <-first_name_physician5_7yr
    last_name_physician <- last_name_physician5_7yr
    street_address_physician <- street_address_physician5_7yr
    city_physician <- city_physician5_7yr
    state_physician <- state_physician5_7yr
    zip_physician <- zip_physician5_7yr

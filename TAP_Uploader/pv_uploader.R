pv_uploader <- function(epoch,vmac) {
  ...
  visit_sched <- pdb_data$visit_sched
  ...
  if (vs == "No") {err <<- "Visit is not scheduled; insufficient data"} else {
    ...
    path_in <- paste0(main_path,"resources/Templates/Previsit/pv2_ptp.docx")
    path_in_proxy <- paste0(main_path,"resources/Templates/Previsit/pv2_proxy.docx")
    ...
    if (visit_type==3) {
      ...
      # ADD PATHS HERE
      path_in <- paste0(main_path,"resources/Templates/Previsit/pv3_ptp.docx")
      path_in_proxy <- paste0(main_path,"resources/Templates/Previsit/pv3_proxy.docx")
    ...
    output <- paste0(out_path,"TAP_",input,"_ptp_letter.docx")
    ...
    if (is.na(proxy_first_name)==FALSE) {
      output_proxy <- paste0(out_path,"TAP_",input,"_proxy_letter.docx")

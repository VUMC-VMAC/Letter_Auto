previsit_uploader <- function(epoch,record) {
  ...
  pdb <- redcapConnection(url = "https://redcap.vanderbilt.edu/api/",
                          token = "773B2F61E8FDCE261E62A3D7986B7DDD", conn, project = 135635)
  pdb_data <- exportReports(pdb, 270113)
  ...
  pdb_datas <- pdb_data[which(as.integer(as.factor(pdb_data[,"redcap_event_name"]))== epoch),]
  
  
  ii <- which(pdb_datas["record_id"]==as.integer(record))
  ...
  if (e > 0) {
    ...
    location_day3<<-""
    location_day3_prox<<-""
    visit_date1 <<- format(pdb_data[,"visit_date1"], "%A, %B %d, %Y")
    visit_time1 <- as.character(pdb_data[,"visit_time1"])
    visit_time1 <<- paste0(gsub(":00$","",visit_time1), "am")
    visit_hours1 <<- as.character(pdb_data["visit_hours1"])
    loc_day1 <<- as.integer(pdb_data["visit_location1"]); X <<- 1
    ...
    if (visit_type==1 | visit_type==3 | visit_type==5 | visit_type==6){
      visit_date2 <<- format(pdb_data[,"visit_date2"], "%A, %B %d, %Y")
      visit_time2 <- as.character(pdb_data[,"visit_time2"])
      visit_time2 <<- paste0(gsub(":00$","",visit_time2),"am")
      visit_hours2 <<- as.character(pdb_data[,"visit_hours2"])
      loc_day2 <<- as.integer(pdb_data["visit_location2"]); X <<- 2
      location <- loc_func(X,loc_day2)
      location_day2 <<- location$ptp; location_day2_prox <<- location$proxy
      day2 <<- paste0("and ",visit_date2," at ",visit_time2)
      add_day2 <<- paste0("The second day of your visit is scheduled for ",visit_date2," at ",visit_time2," and will last approximately ",visit_hours2," hours.")
      add_day2_prox <<- paste0("The second day of ",pronoun_poss," visit is scheduled for ",visit_date2," at ",visit_time2," and will last approximately ",visit_hours2," hours.")
      itin2 <<- "INSTRUCTIONS FOR 2DAY VISIT"
      #fu_time2_7yr <- sub("0","",fu_time2_7yr)
      if (visit_type==3) {
        visit_date3 <<- format(pdb_data[,"visit_date3"], "%A, %B %d, %Y")
        visit_time3 <- as.character(pdb_data[,"visit_time3"])
        visit_time3 <<- paste0(gsub(":00$","",visit_time3),"am")
        visit_hours3 <<- as.character(pdb_data["visit_hours3"])
        loc_day3 <<- as.integer(pdb_data["visit_location3"]); X <- 3
        location <- loc_func(X,loc_day3)
        location_day3 <<- location$ptp; location_day3_prox <<- location$proxy
        day3 <<- paste0("and ",visit_date3," at ",visit_time3)
        add_day3 <<- paste0("The third day of your visit is scheduled for ",visit_date3," at ",visit_time3," and will last approximately ",visit_hours3," hours.")
        add_day3_prox <<- paste0("The third day of ",pronoun_poss," visit is scheduled for ",visit_date3," at ",visit_time3," and will last approximately ",visit_hours3," hours.")
        #fu_time3_7yr <- sub("0","",fu_time3_7yr)
      }
      if (visit_type==4) {
        visit_date4 <<- format(pdb_data[,"visit_date4"], "%A, %B %d, %Y")
        visit_time4 <- as.character(pdb_data[,"visit_time4"])
        visit_time4 <<- paste0(gsub(":00$","",visit_time4),"am")
        visit_hours4 <<- as.character(pdb_data["visit_hours4"])
        location_day4 <<- ""
        add_day4 <<- paste0("The third day of your visit is scheduled for ",visit_date4," at ",visit_time4," and will last approximately ",visit_hours4)
      }
    }
    ...
    transport <- pdb_data["transport_needed"]
    ...
    proxy_require <- pdb_data[,"proxy_require"]
  ...
  path_in <- paste0("C:/Users/sweelyb/Documents/resources/Templates/Previsit/TAP_previsit_ptp.docx")
  temp <- "C:/Users/sweelyb/Documents/resources/Output/previsit_temp.docx"
  path_in_proxy <- paste0("C:/Users/sweelyb/Documents/resources/Templates/Previsit/TAP_previsit_proxy.docx")
  ...
  if (e > 0) {
    ...
    if (X==1) {
      df <- data.frame(
        Day1 = c(paste0("Arrival at ",location1," at ",visit_time1),
                 "Consent Process",
                 "Paperwork Review",
                 ...
                 "Clinical Interview",
                 "Brain MRI",
                 paste0("Study Wrap-up at ",visit_time1," plus ",visit_hours1)))
    ...
    if (X==2) {
      ...
      if (loc_day1==4) {
        df1 <- data.frame(
          Day1 = c(paste0("Arrival at ",location1," at ",visit_time1),
                   ...
                   "Echo",
                   "Brain MRI",
                   paste("Study Wrap-up at ",visit_time1," plus ",visit_hours1)))
      ...
      df2 <- data.frame(
        Day2 = c(paste0("Arrival at ",location2," at ",visit_time2),
                 "Memory Testing",
                 "Lunch",
                 "Clinical Interview",
                 "Heart MRI",
                 "Echo",
                 "Brain MRI",
                 paste0("Study Wrap-up at ",visit_time2," plus ",visit_hours2)))

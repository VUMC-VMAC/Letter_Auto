fb_uploader<<- function(epochh,vmac) {
  ...
  fail <- 0
  dde_data <- dde2_datas[grep(map_id,dde2_datas$record_id),]
  ...
  if (bc == "No" & ec =="No" & nc == "No" | fail) {err <<- "Echo and/or Blood Work not complete; insufficient data"} else {
    ...
    if (e > 3) {
      ...
      df2 <- data.frame(
        ...
        CR = c(round(echo_data$bld_c_chol), round(echo_data$bld_c_hdlc), round(echo_data$bld_c_ldlc), round(echo_data$bld_c_trig), round(echo_data$bld_c_hgba1c,2), round(echo_data$bld_c_insulin,2),round(echo_data$bld_c_glucose), round(echo_data$bld_c_tsh,4), round(echo_data$bld_c_crp,1)),  
    ...
    if (e == 1) {
      ...
      ptp_path<<- paste0("/srv/shiny-server/resources/Templates/Feedback/MAP2_fb_temp_e.docx")
      phys_path<<- paste0("/srv/shiny-server/resources/Templates/Feedback/MAP2_phys_temp_e.docx")
    }
    
    ptp_temp<<- paste0("/app/Output/ptp_temp.docx")
    phys_temp<<- paste0("/app/Output/phys_temp.docx")
    ...
    output<- paste0("/app/Output/feedback_MAP",input,"_",epoch,".docx")
    ...
    if (first_name_physician2!="") {
      ...
      output<- paste0("/app/Output/physician2_letter_MAP_",input,"_",epoch,".docx")
      renderInlineCode(phys_temp, output)
    ...
    if (first_name_physician3!="") {
      ...
      output<- paste0("/app/Output/physician3_letter_MAP_",input,"_",epoch,".docx")
      renderInlineCode(phys_temp, output)
    ...
    if (first_name_physician4!="") {
      ...
      output<- paste0("/app/Output/physician4_letter_MAP_",input,"_",epoch,".docx")
      renderInlineCode(phys_temp, output)
    ...
    if (first_name_physician5!="") {
      ...
      output<- paste0("/app/Output/physician5_letter_MAP_",input,"_",epoch,".docx")
      renderInlineCode(phys_temp, output)

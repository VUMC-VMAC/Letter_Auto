pv_uploader <- function(epoch,vmac) {
  ...
  if (epoch > 0) {visit_sched <- pdb_data$visit_sched} else {visit_sched <- pdb_data$elig_visit_sched}
  if (length(visit_sched)==0) {vs <- "No"} else {if (is.na(visit_sched)) {vs <- "No"} else {vs <- visit_sched}}
  if (vs == "No") {err <<- "Visit is not scheduled; insufficient data"} else {
    err <<- ""
    ...
    if (e > 0) {
      ...
      if (fu_proxy == "Yes") {
        ...
        histor <<- ""
        consent <<- paste0("Consent Statement. This document describes the VMAP Study. You completed this form when you originally came in for your eligibility visit, but we ask that you complete and sign this document again at each follow-up visit. Please read the form thoroughly before the appointment. We will ask you and your study partner, ",proxy_first_name,", to sign the consent form after we review it with you at the appointment.")
        consent_prox <<- paste0("Consent Statement. This document describes the VMAP Study. You and ",first_name," already completed this form at the eligibility visit, but we ask that both of you sign this document again at each follow-up visit. Please read the form thoroughly. If you have questions, please let us know.")
      } else {
      ...
      path_in <- paste0("C:/Users/sweelyb/Documents/resources/Templates/Previsit/MAP_pv2.docx")
      temp <- "C:/Users/sweelyb/Documents/resources/Output/previsit_temp.docx"
      path_in_proxy <- paste0("C:/Users/sweelyb/Documents/resources/Templates/Previsit/MAP_pv2_proxy.docx")
    ...
    if (e > 0) {
      ...
      token <- "496ED1BD518B29CB96B5CFD9C48844FE"
      ...
      formData <- list("token"=token,
                       ...
                       csvDelimiter='',
                       'records[0]'=input,
                       'fields[0]'='visit1_component',
                       'fields[1]'='visit2_component',
                       'fields[2]'='visit3_component',
                       'events[0]'=events[e+1],
                       rawOrLabel='raw',
      ...
      comp <- list()
      comp$Consent <- "Consent Process"
      comp$Quest <- "Paperwork Review"
      comp$CDR <- "Clinical Interview"
      comp$Memory <- "Memory Testing"
      comp$CMR <- "Heart MRI"
      comp$Brain <- "Brain MRI"
      comp$Echo <- "Echo"
      comp$BW <- "Blood Work & Physical Exam"
      comp$LP <- "Lumbar Puncture"
      LP_prep <- "Prep for LP Procedure"
      LP_rest <- "Lumbar Puncture Rest Period"
      ...
      if (X==1) {
        df <- data.frame(
          Day1 = c(paste0("Arrival at ",location1," at ",visit1_time),
                   ...
                   if (as.numeric(components[9])) {LP_rest},
        ...
        #ft <- bg(ft, i=c(2,4,6,8),bg="grey",part = "body")
      ...
      if (X==2) {
        ...
        if (loc_day1==4) {
          df1 <- data.frame(
            Day1 = c(paste0("Arrival at ",location1," at ",visit1_time),
                     ...
                     if (as.numeric(components[2])) {as.character(comp[2])},
                     if (as.numeric(components[3])) {as.character(comp[3])},
                     "Transfer to Vanderbilt University Hospital & Breakfast",
                     if (as.numeric(components[4])) {as.character(comp[4])},
                     if (as.numeric(components[5])) {as.character(comp[5])},
                     if (as.numeric(components[6])) {as.character(comp[6])},
                     if (as.numeric(components[7])) {as.character(comp[7])},
        ...
        df2 <- data.frame(
          Day2 = c(paste0("Arrival at ",location2," at ",visit2_time),
                   if (as.numeric(components[11])) {as.character(comp[1])},
                   if (as.numeric(components[12])) {as.character(comp[2])},
                   if (as.numeric(components[13])) {as.character(comp[3])},
                   if (as.numeric(components[14])) {as.character(comp[4])},
                   if (as.numeric(components[15])) {as.character(comp[5])},
                   if (as.numeric(components[16])) {as.character(comp[6])},
                   if (as.numeric(components[17])) {as.character(comp[7])},
                   "Break",
                   if (as.numeric(components[18])) {as.character(comp[8])},
                   if (as.numeric(components[19])) {LP_prep},
                   if (as.numeric(components[19])) {as.character(comp[9])},
                   if (as.numeric(components[19])) {LP_rest},
        ...
        FT <<- list(ft = ft, ft1 = ft1, ft2 = ft2, ft3 = ft3)
      }
      ...
      if (X == 3) {
        ...
        ft1 <- set_header_labels(ft1, Day1 = "Day 1 Itinerary")
        #ft1 <- bg(ft1, bg="grey",part = "header")
        #ft1 <- bg(ft1, i=c(2,4),bg="grey",part = "body")
        ft1 <- font(ft1,fontname = 'Arial')
        ft1 <- font(ft1,fontname = 'Arial',part = "header")
        ft1 <- fontsize(ft1,size = 13)
        ft1 <- fontsize(ft1,size = 13,part = "header")
        ...
        df3 <- data.frame(
          Day3 = c(paste0("Arrival at ",location3," at ",visit3_time),
                   if (as.numeric(components[21])) {as.character(comp[1])},
                   if (as.numeric(components[22])) {as.character(comp[2])},
                   if (as.numeric(components[23])) {as.character(comp[3])},
                   if (as.numeric(components[24])) {as.character(comp[4])},
                   if (as.numeric(components[25])) {as.character(comp[5])},
                   if (as.numeric(components[26])) {as.character(comp[6])},
                   if (as.numeric(components[27])) {as.character(comp[7])},
                   "Break",
                   if (as.numeric(components[28])) {as.character(comp[8])},
                   if (as.numeric(components[29])) {LP_prep},
                   if (as.numeric(components[29])) {as.character(comp[9])},
                   if (as.numeric(components[29])) {LP_rest},
                   paste0("Study Wrap-up at ",visit3_time," plus ",visit3_hours)))
        ft3 <- flextable(df3) # Day 3 of 3-Day
        ft3 <- set_header_labels(ft3, Day3 = "Day 3 Itinerary")
        #ft3 <- bg(ft3, bg="grey",part = "header")
        #ft3 <- bg(ft3, i=c(2,4),bg="grey",part = "body")

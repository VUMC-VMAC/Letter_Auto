fb_uploader<<- function(epochh,vmac) {
  ...
  ptp_path<<- paste0("/srv/shiny-server/resources/Templates/Feedback/TAP_fb_temp_",epoch,".docx")
  phys_path<<- paste0("/srv/shiny-server/resources/Templates/Feedback/TAP_phys_temp_",epoch,".docx")
  ...
  if (val=="Normal"){val <<-"     2.  No significant"} else {val <<-"     2.  Significant"}
  lv <<- paste0("     1.  ", lv)
  ...
  input <- record
  output<- paste0("/app/Output/feedback_letter_TAP_",input,"_",epoch,".docx")
  ...
  if (is.na(first_name_physician1)==FALSE) {
    ...
    output<- paste0("/app/Output/physician_letter_TAP_",input,"_",epoch,".docx")

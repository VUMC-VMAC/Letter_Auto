previsit_uploader <- function(epoch,vmac) {
  ...
  i <- 1
  #tap_id <- as.character(pdb_data[i,"tap_id"])
  #if (nchar(tap_id)==1) {input <- paste0("00",tap_id)} else if (nchar(tap_id)==2) {input <- paste0("0",tap_id)} else {input <- tap_id}
  ...
  if (visit_type==2 | visit_type==3 | visit_type==4 | visit_type==6 | visit_type==7 | visit_type==8 | visit_type==9){
    ...
    if (visit_type==3 | visit_type==8 | visit_type==4 | visit_type==9) {
    ...
    if (visit_type==4 | visit_type==9) {
      visit4_date <<- format(pdb_data[,"visit4_date"], "%A, %B %d, %Y")
      visit4_time <- as.character(pdb_data[,"visit4_time"])
      visit4_time <<- paste0(gsub(":00$","",visit4_time),"am")
      visit4_hours <<- as.character(pdb_data["visit4_hours"])
      location_day4 <<- ""
  ...
  path_in <- paste0("/srv/shiny-server/resources/Templates/Previsit/TAP_previsit_ptp.docx")
  temp <- "/app/Output/previsit_temp.docx"
  path_in_proxy <- paste0("/srv/shiny-server/resources/Templates/Previsit/TAP_previsit_proxy.docx")
  ...
  output <- paste0("/app/Output/TAP_",input,"_ptp_letter.docx")
  ...
  if (is.na(proxy_first_name)==FALSE) {
    output_proxy <- paste0("/app/Output/TAP_",input,"_proxy_letter.docx")

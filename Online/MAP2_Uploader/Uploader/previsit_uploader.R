      ...
      if (X==1) {
        df <- data.frame(
          Day1 = c(paste0("Arrival at ",location1," at ",visit1_time),
                   if (as.numeric(components[1])) {comp[1]},
                   if (as.numeric(components[2])) {comp[2]},
                   if (as.numeric(components[3])) {comp[3]},
                   "Breakfast",
                   if (as.numeric(components[4])) {comp[4]},
                   if (as.numeric(components[5])) {comp[5]},
                   if (as.numeric(components[6])) {comp[6]},
                   if (as.numeric(components[7])) {comp[7]},
                   "Lunch",
                   if (as.numeric(components[8])) {comp[8]},
                   if (as.numeric(components[9])) {comp[9]},
      ...
      if (X==2 | X==3) {
        ...
        df2 <- data.frame(
          Day2 = c(paste0("Arrival at ",location2," at ",visit2_time),
                   if (as.numeric(components[11])) {comp[1]},
                   if (as.numeric(components[12])) {comp[2]},
                   if (as.numeric(components[13])) {comp[3]},
                   "Breakfast",
                   if (as.numeric(components[14])) {comp[4]},
                   if (as.numeric(components[15])) {comp[5]},
                   if (as.numeric(components[16])) {comp[6]},
                   if (as.numeric(components[17])) {comp[7]},
                   "Lunch",
                   if (as.numeric(components[18])) {comp[8]},
                   if (as.numeric(components[19])) {comp[9]},
